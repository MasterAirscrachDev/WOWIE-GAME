...
public class PlayerAi : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        goal = GameObject.FindGameObjectWithTag("Finish").transform.position + (Vector3.up * 0.5f);
    ...
    void Update()
    {
        ...
        if(Input.GetKey(KeyCode.Alpha1) && Input.GetKeyDown(KeyCode.Alpha9)){
            ravemode = !ravemode;
            Debug.Log("Ravemode: " + ravemode);
        }
        if(ravemode){
            raveindex++;
            if(raveindex > lights.Length - 1){
                raveindex = 0;
            }
        ...
        if(goal.y <= transform.position.y + 0.2f){
            if(CheckPath(Vector3.Lerp(transform.position, goal, 0.5f), goal, 0.1f)){
                ...
                if(!tryingWander && aState == state.Idle){
    ...
    void Movement(Vector3 target){
        ...
        if(currentTarget != target){
            ...
            if(Mathf.Abs(currentTarget.x) < Mathf.Abs(currentTarget.z)){
                ...
                if(CheckPath(midpoint, currentTarget)){
                    dir = 1;
                }
                else{
    ...
    bool CheckPath(Vector3 mid, Vector3 end, float rayTime = 5f){
        ...
        if(Physics.Raycast(transform.position, mid - transform.position, out hit, (mid - transform.position).magnitude)){
            return false;
        }
        else{
            for(float i = 0; i < 1; i += 0.1f){
                ...
                if(!Physics.Raycast(Vector3.Lerp(transform.position, mid, i), Vector3.down, out hit, 0.6f)){
                    return false;
                }
            }
        }
        ...
        if(Physics.Raycast(mid, end - mid, out hit, (end - mid).magnitude)){
            return false;
        }
        else{
            for(float i = 0; i < 1; i += 0.1f){
                ...
                if(!Physics.Raycast(Vector3.Lerp(mid, end, i), Vector3.down, out hit, 0.6f)){
                    return false;
                }
            }
        }
    ...
    IEnumerator Explore(int min = 3, int max = 5){
        ...
        if(aState == state.Idle){
            ...
            while(!found){
                ...
                if(!Physics.Raycast(transform.position, exploreTarget - transform.position) && Physics.Raycast(exploreTarget, Vector3.down, out hit)){
                    if(hit.collider != null){
                        found = true;
                        exploreTarget.y = hit.point.y + 0.5f;
                        Debug.DrawRay(exploreTarget, Vector3.up, Color.green, 2);
                        Debug.Log("Found ExploreTarget");
    ...
    public void SetDistraction(Vector3 pos){
        if(aState != state.Goal){
            ...
            pos.y += 0.4f;
            if(hit.collider == null){
                //round pos.z and pos.x to the nearest 1
                //distraction = new Vector3(Mathf.Round(pos.x), pos.y, Mathf.Round(pos.z));
                distraction = pos;
                //distraction = pos;
...
enum state{
    Goal, Distraction, Idle, Explore, Target
}